...
def main():
    ...
    for step in range(args.num_steps):
        ...
        preds=opred
        print(porg.shape)
        catg_file = CAGDIR + catg_names[step];
        print(catg_file)
        ...
        for ii in range(0, len(catg_IDs)):
            final_pred[:, :, ii] = preddd[0, :, :, catg_IDs[ii]].astype(np.float64)
        ...
        sio.savemat(os.path.join(save_dir1 + "/" + catg_names[step] + '_'), mdict={'pred1': porg}, oned_as='row')
        #sio.savemat(os.path.join(save_dir+"/"+id],names[step]), mdict={'pred1': opred}, oned_as='row')
